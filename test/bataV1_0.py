...
def run(camera_index=0):
    ...
    # สร้างโมเดลใหม่แล้วโหลด weights เข้าไปตรงๆ
    # วิธีนี้ข้าม config ทั้งหมด ไม่สนใจ quantization_config
    model = build_model()
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                mp_drawing.draw_landmarks(
                    ...
                    # กำหนดสีจุดข้อต่อ (Landmarks) เป็นสีขาว
                    # color: (BGR), thickness: ความหนาของขอบ, circle_radius: รัศมีวงกลม
                    landmark_drawing_spec=mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=1, circle_radius=2),
                    # กำหนดสีเส้นเชื่อม (Connections) เป็นสีขาว
                    # color: (BGR), thickness: ความหนาของเส้น
                ...
                # - ถ้าความมั่นใจเกินเกณฑ์ (CONF_THRESHOLD) จะใช้สี (0, 255, 0) คือ สีเขียว 🟢
                # - ถ้าความมั่นใจต่ำ (ทายไม่แม่น) จะใช้สี (0, 165, 255) คือ สีส้ม 🟠
                # box_color = (0, 255, 0) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (106, 174, 123) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (136, 192, 115) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (209, 234, 200) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (58, 93, 35) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                box_color = (84, 125, 57) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (122, 146, 116) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (106, 174, 123) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (126, 157, 79) if best_conf >= CONF_THRESHOLD else (0, 165, 255)
                # box_color = (106, 174, 123) if best_conf >= CONF_THRESHOLD else (0, 165, 255)

                # cv2.rectangle(ภาพ, (มุมซ้ายบน), (มุมขวาขวาต่ำ), สี, ความหนาของเส้น)
                ...
                # (การใส่ความหนาเป็น -1 คือการบอกให้มันถมสีทึบลงไปในกล่องแก)
                cv2.rectangle(
                    frame,
                    (x1, y1 - th - 10),
                    (x1 + tw + 8, y1),
                    box_color, -1      # ใช้สีเดียวกับกรอบ และถมทึบ (-1)
                )
                # (255, 255, 255) คือสีขาวของตัวอักษรแก ส่วนเลข 2 ตัวท้ายคือความหนาตัวหนังสือ
                cv2.putText(
                    frame, label,
                    (x1 + 4, y1 - 5),
            ...
            #     break
            # เปลี่ยนจาก ord("q") เป็นเลข 27 (รหัสของปุ่ม Esc)
